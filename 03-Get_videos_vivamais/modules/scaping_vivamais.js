const puppeteer = require('puppeteer');

/** Contém os algoritmos de scrapping do Vivamais */
class ScrapingVivamais{
    constructor(){}

    // #####################################################################################################################
    // Métodos de extração de dados das páginas de resultados
    // #####################################################################################################################
    
    static async fakeExtraction(videoList){
        const html = await FileUtils.readFile('local_pages_test/results.html');
        const $ = cheerio.load(html); // Carrega o HTML no Cheerio
        // Seleciona os elementos e extrai os dados
        $('#videoSearchResult > .pcVideoListItem').each((index, element) => {
            const $element = $(element);
            const link = $element.find('a').attr('href');
            const regex = /viewkey=([^&]+)/;
            const match = link.match(regex);
            const code = (match)?match[1]:'';
            const title = $element.find('a').attr('title');
            const thumb = $element.find('a img').attr('src');
            const mediabook = $element.find('a img').data('mediabook');
            const metaTags = '';
            let jsonObj = {link,code,title,thumb,mediabook,metaTags}
            videoList.push(jsonObj);
        });
        return true; // has next page?
    }
    
    /** Função de extração dos videos da página de pesquisa */
    static async puppeteerExtraction(url){
        const browser = await puppeteer.launch({headless:false}); // inicializa o navegador
        const page = await browser.newPage(); // cria uma nova página
        try {
            await page.goto(url); // carrega a página de resultados
            await page.evaluate(() => { // simulando uma interação humana de scroll
                window.scrollBy(0, window.innerHeight);
            });
            let linkList = await page.evaluate(() => { // submete a pesquisa js de scrap na página e retorna a lista de videos
                const returnedData = [];
                const dataList = document.querySelectorAll('main > article .post-entry__thumbnail a');
                dataList.forEach(item => {
                    const link = item.href;
                    returnedData.push(link);
                });
                return returnedData; // retorno da consultas js para o node
            });
            await page.close();
            await browser.close();
            return linkList;
        } catch (error) {
            await page.close();
            await browser.close();
            return null;
        }
    }
    
    // #####################################################################################################################
    // Métodos de extração de dados das página (algoritmos de scrapping)
    // #####################################################################################################################
    
    static async fakeScrapeTags(obj) {
        const html = await FileUtils.readFile('local_pages_test/metatags.html');
        const $ = cheerio.load(html); // Carrega o HTML no Cheerio
        // Seleciona os elementos e extrai os dados
        let metaTags = '';
        $('.tagsWrapper .item').each((index, element) => {
            if(index!==0) metaTags+=', '; // separador das metatags
            const $element = $(element);
            metaTags+=$element.text();
        });
        return metaTags; // has next page?
    }
    
    static async puppeteerScrapeData(link) {
        try {
            const browser = await puppeteer.launch({headless:false});
            const page = await browser.newPage();
            await page.goto(link); // operação demorada
            //await page.waitForNavigation();
            const objData =  await page.evaluate(() => {
                const title = document.querySelector('h1.post_single__title');
                const imgLink = document.querySelector('article img.wp-post-image');
                const contentList = document.querySelectorAll('article .post_single__content p');
                contentText='';
                contentList.forEach(element => {
                    contentText+='[p]'+element.innerHTML+'[/p]';
                });
                obj = {
                    'title':title.innerHTML,
                    'imgLink':imgLink.src,
                    contentText
                }
                return obj;
            });
            await page.close();
            await browser.close();
            return objData;
        } catch (error) { return null;}
    }
}
module.exports = ScrapingVivamais;

